...
if [ -z "$YAZI_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <yazi_version> <build_version>"
    echo "Example: $0 25.5.31 1"
    exit 1
fi

PACKAGE_NAME="yazi"
ORIG_TARBALL="${PACKAGE_NAME}_${YAZI_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${YAZI_VERSION}"

echo "Creating Debian/Ubuntu source packages for yazi ${YAZI_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/sxyazi/yazi/archive/refs/tags/v${YAZI_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${YAZI_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as yazi-{VERSION}/ which matches our BUILD_DIR
...
yazi (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${YAZI_VERSION}.
